-- works with any exploit more than sirhurt i suposse
-- credits to original creators 
-- edited by me so it works properly on other games with sirhurt
...
<Z>'..initialSize.Z..'</Z>\
		...
		["LocalScript"] = function(inst)
            ...
            local scr;
            local success_todecompile,result = pcall(decompile,inst)
            if success_todecompile then
                local success_togsub,final_result = pcall(string.gsub,result,"['\"<>&]",xmlReplace)
                if success_togsub then
                    scr = tostring(final_result)
                else
                    scr = string.gsub("-- failed to gsub ("..tostring(final_result)..")","['\"<>&]",xmlReplace) or ""
                end
            else
                scr = string.gsub("-- failed to decompile ("..tostring(result)..")","['\"<>&]",xmlReplace) or ""
            end
			t_ins(buffer, src)
